...
| 趨勢過濾 | yfinance | `^GSPC`（200日均線 + 20日動能） |
...
| 10Y殖利率↑ + 黃金↑（各 > 0.5%） | -1 |
| 美元 UUP 單日 > 1% | -1 |
| S&P 500 近20日報酬 < -6%（動能向下） | -1 |
| **S&P 500 收盤 < 200日均線（結構性空頭）** | **正分砍半再 -2** |
| 高收益利差 > 6%（FRED） | 直接觸發「危機警告」，忽略其他分數 |

結果：≥3→加倉機會、≥1→偏向加倉、≤-1→偏向謹慎、≤-3→考慮減倉、其他→觀望。

> **趨勢過濾設計**：VIX 須「從高點回落」才給逆向加倉分（避免崩盤途中過早抄底）；200日均線是多空總開關，價在均線下時把「買恐慌」的樂觀分數砍半並加警示，避免在下跌趨勢裡接刀。
...
1. ~~**VIX 趨勢偵測**~~：✅ 已實作（2026-06）。VIX 須從5日高點回落>10%才給逆向加倉分。
